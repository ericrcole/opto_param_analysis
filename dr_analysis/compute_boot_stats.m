...
for kk = 1:4
    ...
    for kl = 1:endloop
        ...
        for km = 1:n_bs
            samples_bs{kl,kk,km} = randi(size(dat,1),size(dat,1),1);
        end
    end
...
%             end
            ...
            hold off
            ROI{kl,kk,km} = zeros(size(f_reshape));
            ROI{kl,kk,km}(idx) = 1;
        end
    end
    if kk == plot_animal
...
%% Analyze ROIs
area = nan(6,4,n_bs);
whole_area = nan(4,n_bs);
norm_area = nan(4,6,n_bs);
% Calculate total DR space volume in pixels for each animal
for kk = 1:4
    
        endloop = 6;
    
    whole_space = zeros(size(ROI{1,kk,1}));
    for km = 1:n_bs
        for kl = 1:endloop
            whole_space = whole_space | ROI{kl,kk,km};
            area(kl,kk,km) = sum(sum(ROI{kl,kk,km}));
        end
        whole_area(kk,km) = sum(sum(int64(whole_space)));
        norm_area(kk,:,km) = area(:,kk,km)./whole_area(kk,km);
    end
...
splitA = num2cell(norm_area, [1 2]); %split A keeping dimension 1 and 2 intact
norm_area_bs = vertcat(splitA{:});

[P,TABLE,STATS] = friedman(norm_area_bs, n_bs);
mc = multcompare(STATS);

boot_stats = struct();
boot_stats.norm.p = P;
boot_stats.norm.table = TABLE;
boot_stats.norm.stats = STATS;
boot_stats.norm.mc = mc;
...
for kk = 1:4
    ...
    for kl = 1:endloop
        for jl = 1:endloop
            ...
            similarity(kl,jl,kk) = dice(ROI{kl,kk,km},ROI{jl,kk,km});
            %similarity_oneway(kl,jl,kk) = dice_one_way(ROI{kl,kk},ROI{jl,kk});
            %one-way dice: target is the columns
            
        end
    end
    similarity_bs(:,kk,km) = similarity(1:5,6,kk);
end
end
similarity_bs = permute(similarity_bs, [2 1 3]);

splitA = num2cell(similarity_bs, [1 2]); %split A keeping dimension 1 and 2 intact
similarity_bs_f = vertcat(splitA{:});

[P,TABLE,STATS] = friedman(similarity_bs_f, n_bs);
mc = multcompare(STATS);

%% 
boot_stats.sim.p = P;
boot_stats.sim.table = TABLE;
boot_stats.sim.stats = STATS;
boot_stats.sim.mc = mc;

save('boot_stats.mat','boot_stats')
%%
subplot(2,3,3)
...
[p,tbl,stats]=friedman(norm_area',1);
mcomp = multcompare(stats);

[p,tbl,stats]=friedman(data_to_plot,1);
mcomp = multcompare(stats);
